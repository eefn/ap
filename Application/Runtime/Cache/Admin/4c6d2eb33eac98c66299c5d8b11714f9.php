...
<head>
    ...
    <title>留言管理</title>
    <meta name="keywords" content="H+后台主题,后台bootstrap框架,留言中心主题,后台HTML,响应式后台">
...
<body class="gray-bg">
    ...
    $(document).on('click','.del',function(){ 
            ...
            var tr=$(this).parent('td').parent('tr');
              swal({
                  ...
                  if (isConfirm) {
                    $.ajax({
                      type:'post',
                      url:"<?php echo U('Admin/Guestbook/guestbook_del');?>",
                      data:"del_id="+del_id,
                      dataType:'text',
                      success:function(z){                   
                        if(z==1){
       ...
       $.ajax({
        type:'get',
        url:"<?php echo U('Admin/Guestbook/ajaxPro');?>",
        dataType:'html',
        success:function(z){
          $('.table tbody').html(z);
        }
       ...
       $(document).on('click','.click',function(){
          ...
          var i=$(this).siblings('td').find('.count').text();
          var i=i-1;
             $.ajax({
              type:'get',
              url:"<?php echo U('Admin/Guestbook/click');?>",
              data:'click='+value+'&is_read='+is_read,
              dataType:'html',
              success:function(z){
               if(z==1){
                  $('.click').eq(i).html('<img src="/ap/Public/Admin/img/yes.gif">');
                  $('.click').eq(i).attr('is_read','1'); 
               }else if(z==0){
                $('.click').eq(i).html('<img src="/ap/Public/Admin/img/no.gif">');
                $('.click').eq(i).attr('is_read','0');
               }
